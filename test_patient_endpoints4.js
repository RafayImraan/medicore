...
async function testEndpoints() {
  ...
  try {
    ...
    const labResults = await axios.get(`${baseURL}/patient/${userId}/lab-results`, config);
    console.log('Lab results:', labResults.data.length, 'records');

    // Test prescriptions
    console.log('Testing prescriptions endpoint...');
    const prescriptions = await axios.get(`${baseURL}/patient/${userId}/prescriptions`, config);
    console.log('Prescriptions:', prescriptions.data.length, 'records');

    // Test billing
    console.log('Testing billing endpoint...');
    const billing = await axios.get(`${baseURL}/patient/${userId}/billing`, config);
    console.log('Billing:', billing.data.length, 'records');

    // Test notifications
    console.log('Testing notifications endpoint...');
    const notifications = await axios.get(`${baseURL}/patient/${userId}/notifications`, config);
